 ...
 *
 * デプロイ手順（★AIログは専用スプシに分離＝DATAシートは重くて固まるため）:
 *   1. AIログ専用の新しいスプレッドシートを1つ作る
 *   2. そのスプシで 拡張機能 > Apps Script を開き、このコードを貼る（＝コンテナバインド。SHEET_ID は "" のままでよい）
 *      ※単独スクリプトにする場合は script.google.com で新規作成し、SHEET_ID に新スプシのIDを入れる
 *   3. エディタで setup を1回実行（権限承認＋「AIログ」タブ作成）。ログに ok が出れば成功
 *   4. デプロイ > 新しいデプロイ > 種類=ウェブアプリ / 実行=自分 / アクセス=全員
 *   5. 発行された /exec URL を index.html（と docs/index.html）の LOG_GAS_URL に貼り直す（URLが変わる）
 *
 * 収集: タブ「AIログ」に 1送信=1行。
 ...
 */
// ★AIログ専用スプシに書き込む（DATAシートから分離＝重くて固まるのを回避）。
//   空 "" ＝ このスクリプトを貼ったスプシ自身に書く（コンテナバインド＝推奨・IDの管理不要）。
//   単独スクリプトにするなら、AIログ専用スプシのIDをここに入れる。
var SHEET_ID = "";
var TAB      = "AIログ";
...
// 9「名言」10「層配置」を追加＝振り返り(8)を末尾で正しく区切る＋名言/4層配置を各列へ。
// ラベルは表記ゆれに強い部分文字列（「今日の名言」「セリフの4層配置」等にヒット）。
var LABELS_ = ["今日の目的","使ったモード","途中で変えたモード","使えた単語","難しかった","次回やること","AI活用レベル","今日の振り返り","今日の名言","セリフの4層配置"];
...
function parseLog_(raw){
  ...
  for(n=1;n<=LABELS_.length;n++){
    s = pos[n-1];
    if(s<0){ o[n]=""; continue; }
    e = t.length;
    for(k=n;k<LABELS_.length;k++){ if(pos[k]>=0){ e=pos[k]; break; } }
    seg = t.substring(s, e);
